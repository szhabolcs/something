...
const thingSlice = createSlice({
  ...
  reducers: {
    ...
    setOccuranceForNewPersonalThing: (state, action) => {
      ...
      for (const occurance of state.personalThings.today.new.occurances) {
        if (
          occurance.startTime === action.payload.startTime &&
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(getAllTodaysPersonalThings.fulfilled, (state, action) => {
      state.loading = false;
    ...
    builder.addCase(getAllPersonalThings.fulfilled, (state, action) => {
      state.loading = false;
